...
namespace Variables {
    ...
    namespace Shader {              // Shader default variables
        int   Int       =    0;     // Value will be automatically passed to 'u_VariableInt' uniform in all shaders
        float Float     = 0.0f;     // Value will be automatically passed to 'u_FloatInt' uniform in all shaders
    ...
    struct Transformation { // Scene transformation matrixes ( readonly variables - calculated automatically every frame)
        ...
        void update() {
            Model = glm::rotate(
                glm::rotate(glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -SceneZOffset)), SceneRotation.x, glm::vec3(1.0f, 0.0f, 0.0f)),
                SceneRotation.y, glm::vec3(0.0f, 1.0f, 0.0f));
            View                = Camera.GetViewMatrix();
            ModelView           = View * Model;
            ModelViewInverse    = glm::inverse(ModelView);
            Projection          = glm::perspective(60.0f, float(WindowSize.x) / WindowSize.y, 0.1f, 1000.0f);
...
namespace OpenGL {
    ...
    struct Program {
        Program(GLuint _id) : id(_id) {
            ...
            ModelViewMatrixInverse = glGetUniformLocation(id, "u_ModelViewMatrixInverse");
        ...
        bool hasMVMatrix() const {
            return (MVPMatrix > -1) || (ModelViewMatrix > -1) || (ModelViewMatrixInverse > -1) || (ViewMatrix > -1) || (ModelMatrix > -1);
        }
...
namespace Tools {
    ...
    public:
        ...
        void start() {
            if (query == 0) glCreateQueries(TYPE, 1, &query);
            result = 0;
        ...
        unsigned int getAverage() const {
            return static_cast<unsigned int>(static_cast<double>(resultTotal) / counter + 0.5);
        }
    ...
    // Desc: 
    //-----------------------------------------------------------------------------
    void SaveFramebuffer(GLuint framebufferId = 0, GLsizei width = 0, GLsizei height = 0) {
        if ((width <= 0) || (height <= 0)) {
            GLint viewport[4] = { 0 };
        ...
        char file_name[50] = { 0 };
        sprintf(file_name, "framebuffer_%d_%dx%d_frame_%d.png", framebufferId, width, height, Statistic::Frame::ID);
    ...
    bool WriteFile(void* data, size_t size, const char* fileName) {
        ...
        if (fout) {
            size_t const written = fwrite(data, size, 1, fout);
            fclose(fout);
            return written == size;
        }
        return false;
    ...
    namespace Mesh {
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        bool CreatePlane(float size, int slice_x, int slice_y, std::vector<glm::vec3>& vertices) {
            int const NUM_VERTICES = 4 * slice_x * slice_y;
            if (NUM_VERTICES < 4)
                return false;

            // Compute position deltas for moving down the X, and Z axis during mesh creation
            glm::vec3 const delta = glm::vec3(size / slice_x, size / slice_y, 0.0f);
            glm::vec3 const start = glm::vec3(-size * 0.5f, -size * 0.5f, 0.0f);
            vertices.reserve(NUM_VERTICES);
            for (int x = 0; x < slice_x; x++) {
                for (int y = 0; y < slice_y; y++) {
                    vertices.emplace_back(start + delta * glm::vec3(    x,     y, 0.0f));
                    vertices.emplace_back(start + delta * glm::vec3(x + 1,     y, 0.0f));
                    vertices.emplace_back(start + delta * glm::vec3(x + 1, y + 1, 0.0f));
                    vertices.emplace_back(start + delta * glm::vec3(    x, y + 1, 0.0f));
                }
            }
            return true;
        }
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        bool CreatePlane(unsigned int vertex_density, unsigned int index_density, GLenum mode, std::vector<glm::vec2>& vertices, std::vector<GLuint>& indices) {
            ...
            indices.clear();
            if (index_density == 0) {
                const unsigned int total_indices = (mode == GL_TRIANGLES) ? 6 * (vertex_density - 1) * (vertex_density - 1) : 2 * (vertex_density * vertex_density - 2);
                ...
                const unsigned int total_indices = (mode == GL_TRIANGLES) ? 6 * (index_density - 1) * (index_density - 1) :
                ...
                case GL_TRIANGLES:
                    for (unsigned int y = 0; y < index_density - 1; y++) {
                        const GLuint vindex1 = glm::clamp(static_cast<unsigned int>(glm::round(y * h_index_offset) * vertex_density), 0u, vertex_density * (vertex_density - 1));
                        const GLuint vindex2 = glm::clamp(static_cast<unsigned int>(glm::round((y + 1) * h_index_offset) * vertex_density), 0u, vertex_density * (vertex_density - 1));
                        for (unsigned int x = 0; x < index_density - 1; x++) {
                            const GLuint hindex1 = glm::clamp(static_cast<unsigned int>(x * h_index_offset + 0.5f), 0u, vertex_density - 1);
                            const GLuint hindex2 = glm::clamp(static_cast<unsigned int>((x + 1) * h_index_offset + 0.5f), 0u, vertex_density - 1);
                ...
                case GL_TRIANGLE_STRIP:
                    for (unsigned int y = 0; y < index_density - 1; y++) {
                        ...
                        const size_t endIdx = indices.size() + 2 * index_density - 1;
                        for (unsigned int x = 0; x < index_density; x++) {
                            const GLuint hindex = glm::clamp(static_cast<unsigned int>(x * h_index_offset + 0.5f), 0u, vertex_density - 1);
        ...
        bool CreateCircle(GLenum primitive_type, unsigned int num_triangles, float z,
        ...
        //-----------------------------------------------------------------------------
        bool CreateSphereVertexMesh(std::vector<glm::vec3>& vertices, std::vector<GLushort>& indices, float radius, int sectorCount, int stackCount) {
            ...
            for (int i = 0; i <= stackCount; ++i) {
                float const stackAngle = glm::pi<float>() * 0.5f - i * stackStep; // starting from pi/2 to -pi/2
                float const r = radius * glm::cos(stackAngle);           // r * cos(u)
                float const z = radius * sinf(stackAngle);               // r * sin(u)

                // add (sectorCount+1) vertices per stack
                // the first and last vertices have same position and normal, but different tex coords
                for (int j = 0; j <= sectorCount; ++j) {
                    float const sectorAngle = j * sectorStep; // starting from 0 to 2pi
                    vertices.emplace_back(r * cosf(sectorAngle), r * sinf(sectorAngle), z);
        ...
        // Auth: Song Ho Ahn (song.ahn@gmail.com)
        //-----------------------------------------------------------------------------
        bool CreateSphereVertexMesh(std::vector<glm::vec3>& vertices, float radius, int sectorCount, int stackCount) {
            if ((sectorCount < 3) || (stackCount < 2))
                return false;
            ...
            for (int i = 0; i <= stackCount; ++i) {
                ...
                for (int j = 0; j <= sectorCount; ++j) {
                    float const sectorAngle = j * sectorStep; // starting from 0 to 2pi
                    unique_vertices.emplace_back(r * cosf(sectorAngle), r * sinf(sectorAngle), z);
                    // normal is  : glm::normalize(vertices.back())
                    // texCoord is: glm::vec2(j / sectorCount, i / stackCount);
                }
            }
    ...
    namespace Shader {
        ...
        bool SaveBinaryCode(GLuint programId, const char* fileName) {
            ...
            glGetProgramBinary(programId, binaryLength, nullptr, &binaryFormat, binary);
            bool const result = Tools::WriteFile(binary, binaryLength, fileName);
            free(binary);
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        GLuint CreateShaderFromSource(GLenum shader_type, const char* source, const char* shader_name = nullptr) {
            if (!source)
                return 0;

            switch (shader_type) {
            case GL_VERTEX_SHADER: fprintf(stderr, "%s", shader_name ? shader_name : "vertex shader"); break;
            case GL_FRAGMENT_SHADER: fprintf(stderr, "%s", shader_name ? shader_name : "fragment shader "); break;
            case GL_TESS_CONTROL_SHADER: fprintf(stderr, "%s", shader_name ? shader_name : "tesselation control shader"); break;
            case GL_TESS_EVALUATION_SHADER: fprintf(stderr, "%s", shader_name ? shader_name : "tesselation evaluation shader"); break;
            case GL_GEOMETRY_SHADER: fprintf(stderr, "%s", shader_name ? shader_name : "geometry shader"); break;
            case GL_COMPUTE_SHADER: fprintf(stderr, "%s", shader_name ? shader_name : "compute shader"); break;
            case GL_MESH_SHADER_NV: fprintf(stderr, "%s", shader_name ? shader_name : "mesh shader"); break;
            case GL_TASK_SHADER_NV: fprintf(stderr, "%s", shader_name ? shader_name : "task shader"); break;
            default: return 0;
            }
        ...
        GLuint CreateShaderFromFile(GLenum shader_type, const char* file_name, const char* preprocessor = nullptr) {
            ...
            return CreateShaderFromSource(shader_type, shader_source.c_str(), file_name);
        }
        ...
        void _updateProgramList(GLuint oldProgram, GLuint newProgram) {
            ...
            if ((glGetUniformLocation(newProgram, "u_MVPMatrix") > -1) || 
                ...
                (glGetUniformLocation(newProgram, "u_ModelViewMatrixInverse") > -1) || 
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        bool CreateShaderProgram(GLuint& programId, GLint count, const GLuint* shader_ids) {
            if (!shader_ids)
                return false;
            ...
            if (Variables::ShaderBinaryOutput) {
                glProgramParameteri(pr_id, GL_PROGRAM_BINARY_RETRIEVABLE_HINT, GL_TRUE);
            }
            glLinkProgram(pr_id);
        ...
        bool CreateShaderProgramFromFile(GLuint& programId, const char* vs, const char* tc,
            ...
            if (tbx && !tbx->empty()) {
                glTransformFeedbackVaryings(pr_id, static_cast<GLsizei>(tbx->size()), &(*tbx)[0], GL_INTERLEAVED_ATTRIBS);
            }
            if (Variables::ShaderBinaryOutput) {
                glProgramParameteri(pr_id, GL_PROGRAM_BINARY_RETRIEVABLE_HINT, GL_TRUE);
            }
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        bool CreateMeshShaderProgramFromFile(GLuint& programId, const char* mesh, const char* task,
            const char* fragment, const char* preprocessor = nullptr) {
            ...
            const char* source_file_names[3] = {
                mesh, task, fragment
            };
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        bool CreateComputeShaderProgramFromFile(GLuint& programId, const char* cs, const char* preprocessor = nullptr) {
            if (cs == nullptr)
                return false;

            // Create shader program object
            GLuint pr_id = glCreateProgram();
            GLuint shader_id = CreateShaderFromFile(GL_COMPUTE_SHADER, cs, preprocessor);
            if (shader_id == 0) {
                glDeleteProgram(pr_id);
                return false;
            }
    ...
    namespace Texture {
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        void Show2D(GLuint texID, int x, int y, int width, int height, float scale = 1.0f, int lod = 0, bool border = false) {
            ...
            // Fragment shaders, following formats are supported GL_RGBA8, GL_RGBA8MS, GL_R32UI
            static const char* fsRGBA8 = "#version 400 core\nlayout (location = 0) out vec4 FragColor;\nin vec2 v_TexCoord;\nuniform sampler2D u_Texture;\nuniform int u_Lod = 0;\nuniform float u_Scale = 1.0;\nvoid main(void) {\nFragColor = vec4(textureLod(u_Texture, v_TexCoord, u_Lod).rgb * u_Scale, 1.0);\n}";
            static const char* fsRGBA8MS = "#version 400 core\nlayout (location = 0) out vec4 FragColor;\nin vec2 v_TexCoord;\nuniform sampler2DMS u_Texture;\nuniform float       u_Scale = 1.0;\nvoid main(void) {\nvec3 color = vec3(0.0);\nint numSamples = textureSamples(u_Texture);\nivec2 texCoords = ivec2(v_TexCoord * textureSize(u_Texture));\nfor (int i = 0; i < numSamples; i++)\ncolor += texelFetch(u_Texture, texCoords, i).rgb * u_Scale;\nFragColor = vec4(color/u_NumSamples, 1.0);\n}";
            static const char* fsR32UI = "#version 400 core\nlayout (location = 0) out vec4 FragColor;\nin vec2 v_TexCoord;\nuniform usampler2D u_Texture;\nuniform int u_Lod = 0;\nuniform float u_Scale = 1.0;\nvoid main(void) {\nuint color = textureLod(u_Texture, v_TexCoord, u_Lod).r;\nFragColor = vec4(float(color) * u_Scale, 0.0, 0.0, 1.0);\n}";
            ...
            // Compile shaders
            if ((programs[programIdx] == 0) && !Shader::CreateShaderProgramFromSource(programs[programIdx], 2, SHADER_TYPES, sources)) {
            ...
            GLboolean const depthTest = glIsEnabled(GL_DEPTH_TEST);
            if (depthTest) {
                glDisable(GL_DEPTH_TEST);
            }
            glUseProgram(programs[programIdx]);
            glUniform1i(glGetUniformLocation(programs[programIdx], "u_Texture"), 0);
            glUniform1i(glGetUniformLocation(programs[programIdx], "u_NumSamples"), numSamples);
            glUniform1i(glGetUniformLocation(programs[programIdx], "u_Lod"), lod);
            glUniform1f(glGetUniformLocation(programs[programIdx], "u_Scale"), scale);
            glUniform4fv(glGetUniformLocation(programs[programIdx], "Vertex"), 4, vertices);
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        void Show3D(GLuint texID, GLint slice, GLint x, GLint y, GLsizei width, GLsizei height, float scale = 1.0f, int lod = 0, bool border = false) {
            static const GLenum SHADER_TYPES[] = { GL_VERTEX_SHADER, GL_FRAGMENT_SHADER };
            // Vertex shader
            static const char* vs = "#version 400 core\nuniform vec4 Vertex[4];\nout vec2 v_TexCoord;\nvoid main(void) {\nv_TexCoord  = Vertex[gl_VertexID].zw;\ngl_Position = vec4(Vertex[gl_VertexID].xy, 0.0, 1.0f);\n}";
            // Fragment shader GL_RGBA
            static const char* fsRGBA8 = "#version 450 core\nlayout (location = 0) out vec4 FragColor;\nin vec2 v_TexCoord;\nuniform sampler3D u_Texture;\nuniform float u_SliceZ = 0.0;\nuniform int u_Lod = 0;\nuniform float u_Scale = 1.0;\nvoid main(void) {\nFragColor = vec4(textureLod(u_Texture, vec3(v_TexCoord, u_SliceZ), u_Lod).rrr * u_Scale, 1.0);\n}";
            ...
            if (program == 0) {
                ...
                if (!Shader::CreateShaderProgramFromSource(program, 2, SHADER_TYPES, sources)) {
        ...
        // Desc:
        //-----------------------------------------------------------------------------
        void ShowCube(GLuint texID, GLint x, GLint y, GLsizei width, GLsizei height) {
            static const GLenum SHADER_TYPES[] = { GL_VERTEX_SHADER, GL_FRAGMENT_SHADER };
            ...
            if (program == 0) {
                ...
                glProgramUniform1i(program, glGetUniformLocation(program, "u_Texture"), 0);
                glProgramUniform4f(program, glGetUniformLocation(program, "u_Color"), 1.0f, 1.0f, 1.0f, 1.0f);
                glProgramUniform3fv(program, glGetUniformLocation(program, "u_Vertices"), 36, vertices);
                glProgramUniform3fv(program, glGetUniformLocation(program, "u_TexCoords"), 36, texCoords);
            }
            ...
            glGetIntegerv(GL_CURRENT_PROGRAM, &curProgramId);
            GLboolean const depthTest = glIsEnabled(GL_DEPTH_TEST);
            GLboolean const blending  = glIsEnabled(GL_BLEND);
            if (depthTest) {
                glDisable(GL_DEPTH_TEST);
            }
            ...
            glUniform4f(glGetUniformLocation(program, "u_Scale"), width / 4.0f, height / 3.0f, 2.0f / (vp.z - vp.x), 2.0f / (vp.w - vp.y));
        ...
        //-----------------------------------------------------------------------------
         ...
         // Desc:
         //-----------------------------------------------------------------------------
        inline void ShowDepth(GLuint texID, GLint x, GLint y, GLsizei width, GLsizei height, float nearZ, float farZ) {
            static const GLenum SHADER_TYPES[] = { GL_VERTEX_SHADER, GL_FRAGMENT_SHADER };
            // Vertex shader
            static const char* vs = "#version 450 core\nuniform vec4 Vertex[4];\nout vec2 v_TexCoord;\nvoid main(void) {\nv_TexCoord  = Vertex[gl_VertexID].zw;\ngl_Position = vec4(Vertex[gl_VertexID].xy, 0.0, 1.0f);\n}";
            ...
            glUniform1i(glGetUniformLocation(programs[programIdx], "u_NumSamples"), numSamples);
            glUniform4fv(glGetUniformLocation(programs[programIdx], "Vertex"), 4, vertices);
            glUniform1f(glGetUniformLocation(programs[programIdx], "u_NearZ"), nearZ);
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        bool GetImageData(const char* filename, std::vector<unsigned char>& data, glm::ivec3* resolution = nullptr) {
            int width = 0, height = 0, numChannels = 0;
            unsigned char* pixels = stbi_load(GetFilePath(filename).c_str(), &width, &height, &numChannels, 0);
            if (pixels) {
                data.reserve(data.size() + width * height * numChannels);
                data.insert(data.end(), &pixels[0], &pixels[width * height * numChannels]);
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        GLuint CreateFromFile(const char* filename, bool mipmapped = true, glm::ivec3 * resolution = nullptr) {
            GLuint texId = 0;
            int width = 0, height = 0, numChannels = 0;
            unsigned char* pixels = stbi_load(GetFilePath(filename).c_str(), &width, &height, &numChannels, 0);
            if (pixels && (numChannels == 3 || numChannels == 4)) {
                auto const log = glm::floor_log2(glm::max(width, height));
                int const numLevels = !mipmapped ? 1 : (log + 1);
                fprintf(stderr, "NUM LEVELS %d\n", numLevels);

                glCreateTextures(GL_TEXTURE_2D, 1, &texId);
                glTextureParameteri(texId, GL_TEXTURE_MIN_FILTER, mipmapped ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
                glTextureParameteri(texId, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
                glTextureParameteri(texId, GL_TEXTURE_WRAP_S, GL_REPEAT);
                glTextureParameteri(texId, GL_TEXTURE_WRAP_T, GL_REPEAT);
                glTextureStorage2D(texId, numLevels, GL_RGBA8, width, height);
                glTextureSubImage2D(texId, 0, 0, 0, width, height, (numChannels == 3) ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE, pixels);
                if (numLevels > 1) {
        ...
        GLuint CreateColored(GLuint texture_id) {
            ...
            for (int level = 0; level < max_level; level++) {
                ...
                glTextureSubImage2D(colored_texture_id, level, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, &texels[0]);
            }
        ...
        GLuint CreateSimpleTexture(GLint width, GLint height) {
            ...
            glTextureSubImage2D(tex_id, 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, &texels[0]);
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        void Create2D(GLuint& texture, GLenum format, glm::ivec2 const& resolution, GLsizei numSamples = 0) {
            glDeleteTextures(1, &texture);
            glCreateTextures(numSamples == 0 ? GL_TEXTURE_2D : GL_TEXTURE_2D_MULTISAMPLE, 1, &texture);
            glTextureParameteri(texture, GL_TEXTURE_WRAP_S, GL_REPEAT);
            glTextureParameteri(texture, GL_TEXTURE_WRAP_T, GL_REPEAT);
            if (numSamples > 0) {
                glTextureStorage2DMultisample(texture, numSamples, format, resolution.x, resolution.y, GL_TRUE);
            } else {
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        void Create3D(GLuint& texture, GLenum format, glm::ivec3 const& resolution, bool mipmap = false) {
            glDeleteTextures(1, &texture);
            ...
            glTextureParameteri(texture, GL_TEXTURE_WRAP_S, GL_REPEAT);
            glTextureParameteri(texture, GL_TEXTURE_WRAP_T, GL_REPEAT);
            glm::ivec3 const levels    = glm::log2(resolution);
            GLsizei    const max_level = mipmap ? glm::max(glm::max(levels.x, levels.y), levels.z) : 1;
            glTextureStorage3D(texture, max_level, format, resolution.x, resolution.y, resolution.z);
            if (mipmap) {
    ...
    namespace Noise {
        ...
        float SmoothNoise(float x, float y) {
            ...
            return fractY * (fractX * Noise[y1][x1] + (1.0f - fractX) * Noise[y1][x2]) + (1.0f - fractY) * (fractX * Noise[y2][x1] + (1.0f - fractX) * Noise[y2][x2]);
        }
        ...
        // Desc: 
        //-----------------------------------------------------------------------------
        bool GenerateMarblePattern(std::vector<glm::u8vec3> pixels, unsigned int width, unsigned int height, int turbSize = 32, float turbPower = 5.0f, float xPeriod = 5.0f, float yPeriod = 10.0f) {
            GenerateNoise();
    ...
    namespace GUI {
        ...
        class Graph {
        public:
            Graph(const char* name, glm::ivec2 size, TYPE* counters, const std::vector<char*>& titles)
                : name(name), size(size.x, size.y), counters(counters), titles(titles), history(100.0f) {
            ...
            void update(bool showHistory = false) {
                ...
                for (int i = 0; i < titles.size(); i++) {
                    ...
                    ImGui::PushStyleColor(ImGuiCol_PlotHistogram, (ImVec4)ImColor(0.02f, 0.62f, 0.82f, 1.0f));
                    ImGui::ProgressBar(values[i] / float(sumValues), ImVec2(0.0f, 0.0f));
                ...
                if (showHistory) {
                    ImGui::SetNextItemWidth(glm::max(90.0f, size.x / IMGUI_RESIZE_FACTOR - 55.0f) * IMGUI_RESIZE_FACTOR);
                    ImGui::SliderFloat("History", &history, 10, 1000, "%.0f frames");
                    for (int i = 0; i < titles.size(); i++) {
                    ...
                    ImPlot::SetNextPlotLimitsY(0, maxValue);
                    if (ImPlot::BeginPlot("##Rolling", nullptr, nullptr, size, 0, rt_axis, rt_axis)) {
                        for (int i = 0; i < titles.size(); i++) {
                            ImPlot::PlotLine(titles[i], &data[i].Data[0].x, &data[i].Data[0].y, data[i].Data.size(), 0, 2 * sizeof(float));
                        }
                        ImPlot::EndPlot();
                    }
                }
    ...
    // Simple interface for multi-algorithm examples
    class IAlgorithm {
    public:
        using UniquePtr = std::unique_ptr<IAlgorithm>;

    public:
        virtual void render() {}
        virtual void present(void (*drawCallback)(void)) {}
        virtual void showInfo() {}
        virtual void updateShaders() {}
        virtual void reset() {}
        virtual void enable() {}
        virtual void resizeWindow(const glm::ivec2& windowSize) {}
