...
namespace Callbacks {
    ...
    namespace GUI {
        ...
        void Show(void * user) {
            ...
            int heightOffset = 10;
            if (User::ShowGUI) {
                ImGui::SetNextWindowPos(ImVec2(10, heightOffset) * IMGUI_RESIZE_FACTOR, ImGuiCond_Always);
                heightOffset += User::ShowGUI() + 5;
            }

            ImGui::SetNextWindowPos(ImVec2(10, heightOffset) * IMGUI_RESIZE_FACTOR, ImGuiCond_Once);
            ImGui::SetNextWindowSize(ImVec2(220, 100) * IMGUI_RESIZE_FACTOR, ImGuiCond_Always);
            ...
            ImGui::SetNextWindowPos(ImVec2(10, heightOffset + 105) * IMGUI_RESIZE_FACTOR, ImGuiCond_Once);
            ImGui::SetNextWindowSize(ImVec2(220, 125) * IMGUI_RESIZE_FACTOR, ImGuiCond_Always);
            ...
            ImGui::SetNextWindowPos(ImVec2(10, heightOffset + 235) * IMGUI_RESIZE_FACTOR, ImGuiCond_Once);
            ImGui::SetNextWindowSize(ImVec2(220, 220) * IMGUI_RESIZE_FACTOR, ImGuiCond_Always);
            ...
            ImGui::PlotLines("", gpu_times, IM_ARRAYSIZE(gpu_times), gpu_times_offset, overlay, 0.0f, gpu_max_time, ImVec2(205, 50) * IMGUI_RESIZE_FACTOR);
            ...
            ImGui::PlotLines("", cpu_times, IM_ARRAYSIZE(cpu_times), cpu_times_offset, overlay, 0.0f, cpu_max_time, ImVec2(205, 50) * IMGUI_RESIZE_FACTOR);

            if (Variables::ShowMemStat) {
                ImGui::Text("Total GPU memory %d KB", Statistic::GPUMemory::TotalMemory);
                ...
                ImGui::ProgressBar(Statistic::GPUMemory::AllocatedMemory / static_cast<float>(Statistic::GPUMemory::TotalMemory), ImVec2(80.0f, 0.0f) * IMGUI_RESIZE_FACTOR);
                ImGui::SameLine(0.0f, ImGui::GetStyle().ItemInnerSpacing.x);
                ImGui::Text("%u MB allocated", static_cast<unsigned int>(Statistic::GPUMemory::AllocatedMemory / 1024));
                ImGui::ProgressBar(Statistic::GPUMemory::FreeMemory / static_cast<float>(Statistic::GPUMemory::TotalMemory), ImVec2(80.0f, 0.0f) * IMGUI_RESIZE_FACTOR);
                ImGui::SameLine(0.0f, ImGui::GetStyle().ItemInnerSpacing.x);
                ImGui::Text("%u MB free", static_cast<unsigned int>(Statistic::GPUMemory::FreeMemory / 1024));
                ...
                //ImGui::SameLine(0.0f, ImGui::GetStyle().ItemInnerSpacing.x);
                //ImGui::Text("%u MB evicted", static_cast<unsigned int>(Statistic::GPUMemory::EvictedMemory / 1024));
            }
            ImGui::End();

            // Create zoom window
            ImGui::SetNextWindowPos(ImVec2(10, heightOffset + 460) * IMGUI_RESIZE_FACTOR, ImGuiCond_Once);
            ImGui::SetNextWindowSize(ImVec2(220, 125)* IMGUI_RESIZE_FACTOR, ImGuiCond_Always);
            ImGui::Begin("Inspector");
            if (ImGui::Button("take screenshot", ImVec2(204, 0) * IMGUI_RESIZE_FACTOR))
                Tools::SaveFramebuffer();

            ImVec4 const color = ImColor(Variables::Inspector::pixel.r, Variables::Inspector::pixel.g, Variables::Inspector::pixel.b, 255);
            ImGui::PushStyleColor(ImGuiCol_Button, color);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, color);
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, color);
            ImGui::Button(" ", ImVec2(19.0f, 0.0f) * IMGUI_RESIZE_FACTOR);
            ImGui::PopStyleColor(3);
            ImGui::SameLine(0.0f, ImGui::GetStyle().ItemInnerSpacing.x);
            ImGui::Text("R: %d G: %d B: %d A: %d", Variables::Inspector::pixel.r, Variables::Inspector::pixel.g, Variables::Inspector::pixel.b, Variables::Inspector::pixel.a);
            ...
            if (ImGui::CollapsingHeader("Magnifier", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                ImGui::SliderInt("zoom", &Variables::Inspector::Zoom::Factor, 2, 10, "%d");
            }
            ImGui::End();
    ...
    void KeyboardChanged(GLFWwindow* window, int key, int scancode, int action, int mods) {
        static float const CameraSensitivity = 0.05f;
        ...
        case GLFW_KEY_Z:
            if (Variables::ModelTransformEnabled) {
                Variables::Transform.SceneZOffset += (mods == GLFW_MOD_SHIFT) ? -0.5f : 0.5f;
                Variables::Transform.update();
            }
            return;
    ...
    void MouseButtonChanged(GLFWwindow* window, int button, int action, int mods) {
        ...
        bool const  clicked       = (action == GLFW_PRESS);
        ...
        if (clicked) {
            ...
            case GLFW_MOUSE_BUTTON_RIGHT:
                ...
                {
                    double const diff_ms = std::chrono::duration <double, std::milli>(now - lastClickTime[GLFW_MOUSE_BUTTON_RIGHT]).count();
                    if (Variables::Inspector::Zoom::Enabled && (diff_ms < 200))
                        Variables::Inspector::Zoom::Center = glm::round(Variables::MousePosition);
                }
            ...
            case GLFW_MOUSE_BUTTON_MIDDLE:
                ...
                glReadPixels(Variables::MousePosition.x, Variables::WindowSize.y - Variables::MousePosition.y, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, &Variables::Inspector::pixel);
                break;
            }
        }
        else {
            Variables::MouseLeftPressed = Variables::MouseRightPressed = Variables::MouseMiddlePressed = false;
        }

        if (User::MouseButtonChanged)
            User::MouseButtonChanged(button, action);
    }
    ...
    void MousePositionChanged(GLFWwindow* window, double x, double y) {
        static float const SceneMovementSensitivity  = 0.9f;
        ...
        bool const middleMouse       = Variables::MouseMiddlePressed && !ImGui::GetIO().MouseDownOwned[2];
        glm::vec2 const lastMousePos = Variables::MousePosition;
        Variables::MousePosition     = glm::vec2(x, y);
        glm::vec2 const change       = Variables::MousePosition - lastMousePos;
        if (leftMouse) {
            if (!Variables::ModelTransformEnabled) {
                Variables::Transform.Camera.rotateAroundCenter(change.x, change.y);
            }
            else {
                Variables::Transform.SceneRotation.x += SceneMovementSensitivity * change.y;
                Variables::Transform.SceneRotation.y += SceneMovementSensitivity * change.x;
            }
            Variables::Transform.update();
        }
        if (rightMouse) {
        ...
        if (User::MousePositionChanged)
            User::MousePositionChanged(x, y);
        // TODO: Currently disabled because of bug in ImGui 
        //if (middleMouse) {
        //    glReadPixels(Variables::MousePosition.x, Variables::MousePosition.y, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, &Variables::Inspector::pixel);
        //}
    }
    ...
    PrintOGLDebugLog(GLenum source, GLenum type, GLuint id, GLenum severity,
        ...
        if ((id == 0x20070) && (severity == GL_DEBUG_SEVERITY_LOW))
            return;
        if ((id == 0x20071) && (severity == GL_DEBUG_SEVERITY_NOTIFICATION) && (type == GL_DEBUG_TYPE_OTHER)) // daam4408
            return;
...
//-----------------------------------------------------------------------------
void ShowMagnifier() {
    if (!Variables::Inspector::Zoom::Enabled)
        return;

    static GLuint               s_MagTexture              = 0;
    static GLsizei const        s_MagWindowResolution     = 200 * WINDOW_RESIZE_FACTOR;
    ...
    GLsizei const magTextureResolution     = s_MagWindowResolution / Variables::Inspector::Zoom::Factor;
    ...
    GLint const invY = Variables::WindowSize.y - Variables::Inspector::Zoom::Center.y;
    GLint const x = glm::clamp(Variables::Inspector::Zoom::Center.x - magTextureHalfResolution, 0, Variables::WindowSize.x - magTextureResolution);
    ...
    GLint const x1 = glm::clamp(Variables::Inspector::Zoom::Center.x - s_MagWindowHalfResolution, 0, Variables::WindowSize.x - s_MagWindowResolution);
    GLint const y1 = glm::clamp(invY - s_MagWindowHalfResolution, 0, Variables::WindowSize.y - s_MagWindowResolution);
    Tools::Texture::Show2D(s_MagTexture, x1, y1, s_MagWindowResolution, s_MagWindowResolution, 1.0f, 0, true);
}
...
int common_main(int window_width, int window_height, const char* window_title,
    ...
    Variables::WindowSize = glm::ivec2(window_width * WINDOW_RESIZE_FACTOR, window_height * WINDOW_RESIZE_FACTOR);
        ...
        // Update transformations and default variables if used
        {
            for (size_t i = 0; i < OpenGL::programs.size(); i++) {
                ...
                if (program.ModelMatrix > -1)
                    glUniformMatrix4fv(program.ModelMatrix, 1, GL_FALSE, &Variables::Transform.Model[0][0]);
                if (program.ViewMatrix > -1)
                    glUniformMatrix4fv(program.ViewMatrix, 1, GL_FALSE, &Variables::Transform.View[0][0]);
                ...
                if (program.ModelViewMatrixInverse > -1)
                    glUniformMatrix4fv(program.ModelViewMatrixInverse, 1, GL_FALSE, &Variables::Transform.ModelViewInverse[0][0]);
